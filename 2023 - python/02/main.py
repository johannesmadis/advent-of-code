...
for line in input.split("\n"):
    ...
    for game_set in sets:
        pairs = game_set.split(", ")
        for pair in pairs:
            num, color = pair.split(" ")
            if int(num) > counter[color]:
                counter[color] = int(num)

    power = counter["red"] * counter["blue"] *counter["green"]
    game_cubes.append(power)
    
    if counter["red"] <= max_red and counter["blue"] <= max_blue and counter["green"] <= max_green:
        games.append(game_index)

print(sum(games))
print(sum(game_cubes))